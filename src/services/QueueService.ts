...
export class QueueService {
    ...
    constructor(
        private readonly sendMessage: (params: { chat_id: string; text: string; parse_mode?: string }) => Promise<void>
    ) {
        ...
        try {
            ...
            for (const message of messages) {
                ...
                try {
                    ...
                    logger.error({ error, messageId: message.id }, "Failed to process queued message");
                    ...
                    if (message.attempts < this.maxAttempts) {
                        ...
                        logger.info(
                            { messageId: message.id, nextRetryIn: nextRetry / 1000 },
